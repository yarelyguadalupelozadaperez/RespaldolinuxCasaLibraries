 ...
 * @see Zend_Db
 */
include_once 'PgsqlConnectionV2.php';
include_once 'Table.php';

class PgsqlQueries extends PgsqlConnectionV2
{
    ...
    function queryDb($table, $fieldsArray = array(), $joins = "", $where = "", $paramsArray = array()) {
		// Construction of the fields string
		$fields = "";
		...
		$parameters = str_replace ( "\'", "'", $parameters );
		$parameters = str_replace ( '|', '"', $parameters );
		$stmt = $this->db->query('SELECT ' . $fields . 'FROM ' . $table . ' ' . $joins . ' WHERE ' . $where, $paramsArray);
		$rows = $stmt->fetchAll();
